...
The python-nmap.py uses the official Nmap engine. It is faster, more accurate, and provides more reliable state detection than raw socket scripts.
```bash
python python-nmap.py --host scanme.nmap.org --ports 22,80,443
```
2. Raw Versions (Async/Threads)
For lightweight scanning without external dependencies (except tabulate):
# Using Asyncio (default)
```bash
python raw.py --host 192.168.1.1 --ports 1-1024 --mode async
```

# Using Threads
```bash
python raw.py --host 127.0.0.1 --ports 80,443,8080 --mode thread
```
⚙️ Arguments
